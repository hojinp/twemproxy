 ...
 */
char *
redis_parse_peer_msg_get_key(struct msg *r) {
    struct mbuf *xbuf;
    char *key = NULL;
...
void redis_parse_req(struct msg *r) {
    ...
    log_hexdump(LOG_VERB, b->pos, mbuf_length(b), "parsed req %" PRIu64
                                                  " res %d "
...
void redis_parse_rsp(struct msg *r) {
    ...
    log_hexdump(LOG_VERB, b->pos, mbuf_length(b), "parsed rsp %" PRIu64
                                                  " res %d "
                                                  "type %d state %d rpos %d of %d",
                r->id, r->result, r->type,
                r->state, (int)(r->pos - b->pos), (int)(b->last - b->pos));
...
void redis_pre_coalesce(struct msg *r) {
    ...
    switch (r->type) {
        ...
        default:
             ...
             */
            mbuf = STAILQ_FIRST(&r->mhdr);
            log_hexdump(LOG_ERR, mbuf->pos, mbuf_length(mbuf),
                        "rsp fragment "
                        "with unknown type %d",
                        r->type);
            pr->error = 1;
...
void redis_post_coalesce_del_or_touch(struct msg *request) {
    struct msg *response = request->peer;
    rstatus_t status;
